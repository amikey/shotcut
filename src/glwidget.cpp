...
static void uploadTextures(QOpenGLContext* context, SharedFrame& frame, GLuint texture[])
{
    ...
    GLint format = (textureCount == 1)? GL_RGB : GL_LUMINANCE;
    ...
    check_error(f);
    f->glTexImage2D   (GL_TEXTURE_2D, 0, format, width, height, 0,
...
void GLWidget::mouseMoveEvent(QMouseEvent* event)
{
    ...
    if (m_frameRenderer && m_frameRenderer->getDisplayFrame().is_valid()) {
        ...
        if (image) {
            ...
            drag->setPixmap(QPixmap::fromImage(qimage.scaled(45 * MLT.profile().dar(), 45)));
        }
    }
...
#endif
    (*thread) = new RenderThread(function, data, quickWindow()->openglContext(), &m_offscreenSurface);
    (*thread)->start();
}
...
QImage GLWidget::image() const
{
    ...
    if (frame.is_valid()) {
        ...
        if (image) {
            ...
            memcpy(result.scanLine(0), image, width * height * 3);
            return result;
        }
    }
    return QImage();
